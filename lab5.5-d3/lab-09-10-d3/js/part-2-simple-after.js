...
var data = [
	...
	[
		// Votes for kiwis across I School group
	        { x: 0, y: 10 },
	        { x: 1, y: 12 },
	        { x: 2, y: 19 },
	        { x: 3, y: 23 },
...
// Set up the scales
var xDomainRange = d3.range(data[0].length);
var xScale = d3.scale.ordinal()
	.domain(xDomainRange)
	.rangeRoundBands([margin.left, svgWidth], .05); // Corrected bar spacing
...
var yAxisScaleDomain = d3.max(data, function(d) {
	return d3.max(d, function(d) {
		return d.y;
	});
})
var yAxisScale = d3.scale.linear()
										.domain([0,	yAxisScaleDomain])
										.range([chartHeight, margin.top]);
